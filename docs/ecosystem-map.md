...
| `public/JSS*.md` (inside Akulearn_docs) | JSS1–JSS3 starter content bundles (9 files): Mathematics, English Language, Basic Science × 3 levels + Social Studies | Markdown | ✅ Created — push to Aku-Content via `generate-jss-content-starters.yml` |
| `pipeline/` (inside Akulearn_docs) | Tracked content generation pipeline: `textbook_generator.py`, `flashcard_generator.py`, `quiz_generator.py`, `bece_scraper.py` | Python 3.11 | ✅ Created — extend and run locally or in CI |
| [Aku-Content](https://github.com/oumar-code/Aku-Content) | Dedicated content delivery repo (Git LFS for binaries) — consumed by Akudemy and Aku-EdgeHub | — | ✅ Repo created — JSS starters ready to push via `generate-jss-content-starters.yml` |
...
- [x] **JSS1–JSS3 starter content bundles created** (9 Markdown files in `public/JSS*.md` — Mathematics, English Language, Basic Science × 3 levels + Social Studies)
- [x] **Content generation pipeline created** (`pipeline/textbook_generator.py`, `pipeline/flashcard_generator.py`, `pipeline/quiz_generator.py`, `pipeline/bece_scraper.py`)
- [x] **GitHub Actions workflow created** (`generate-jss-content-starters.yml`) — pushes JSS starters + generated content to Aku-Content
- [x] **Akudemy seed_content.py created** (`docs/service-migrations/scaffolds/Akudemy/scripts/seed_content.py`)
- [x] **MCP strategy documented** (`docs/mcp/index.md`) — Wikipedia, YouTube Transcript, Khan Academy, NERDC PDF
...
| Priority | Content Type | Class Level | Status | Action |
|----------|-------------|------------|--------|--------|
| **P1** | Textbook starters (MD) | JSS1–JSS3 | ✅ Created | Trigger `generate-jss-content-starters.yml` (tier1_starters) |
| **P1** | LLM-generated chapters (JSON+MD) | JSS1–JSS3 | ⏳ Ready to run | Run `pipeline/textbook_generator.py --all-jss` |
| **P1** | LLM-generated chapters (JSON+MD) | SS1–SS3 (gaps) | ⏳ Ready to run | Run `pipeline/textbook_generator.py --all-ss` |
| **P2** | BECE past questions | JSS3 | ⏳ Ready to run | Run `pipeline/bece_scraper.py --all` |
| **P2** | Flashcard decks | JSS1–SS3 | ⏳ Ready to run | Trigger `generate-jss-content-starters.yml` (tier2_flashcards) |
| **P2** | Topic quizzes | JSS1–SS3 | ⏳ Ready to run | Trigger `generate-jss-content-starters.yml` (tier2_quizzes) |
| **P3** | Wikipedia .zim | All | ⏳ 1 command | `wget` from kiwix.org (see `docs/mcp/index.md`) |
| **P4** | Multilingual translations | JSS1–SS3 | ⏳ Needs Tier 1 | Phase 2 |
| **P5** | AR .glb / VR .unitypackage | All | ⏳ Blender/Unity | Phase 3 |
