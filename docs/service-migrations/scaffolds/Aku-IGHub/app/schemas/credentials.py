...
class CredentialIssueRequest(BaseModel):
    ...
    claims: dict[str, Any] = Field(
        ..., description="Domain-specific credential claims; must not contain raw PII"
    )
...
class CredentialVerifyResponse(BaseModel):
    ...
    credential_id: str
    valid: bool = Field(
        ..., description="True only when signature, expiry, and revocation checks all pass"
    )
    ...
    expiry_date: datetime | None = None
    revoked: bool = Field(
        False, description="True if the credential appears on the revocation registry"
    )
